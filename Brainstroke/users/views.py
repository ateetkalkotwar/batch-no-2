...
from users.forms import UserRegistrationForm, BrainDataForm
from users.models import UserRegistrationModel, BrainStrokeDataModel
import numpy as np
import pandas as pd
...
def UserAddData(request):
    if request.method == 'POST':
        form = BrainDataForm(request.POST)
        if form.is_valid():
            print('Data is Valid')
            form.save()
...
def UserDataView(request):
    data_list = BrainStrokeDataModel.objects.all()
...
def BrainStroke(request):
    return render(request, 'users/BrainStrokeDisease.html')


def BrainStrokePrediction(request):
    import pickle
    ...
    return render(request, 'users/BrainStrokePrediction.html', {"prediction":pred})
